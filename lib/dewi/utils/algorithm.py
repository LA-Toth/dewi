...
def qsort(lst: list, func: (callable, None)=None):
    if len(lst) <= 1:
        return lst
    if func is None:
...
def unique(seq: Sequence):
    return list(_unique(seq))


def list_clear(seq: Sequence):
    """Removes empty elements from list"""
    return [x for x in seq if x or len(x)]
...
#
def levenhstein(str1: str, str2: str, w, a, s, d):
    len1 = len(str1)
    len2 = len(str2)
    l = len1
    row0 = [0] * (len2 + 1)
...
def get_similar_names_to(name: str, possible_names: Sequence, max_result_count: int=6,
                         w: int=0, a: int=2, s: int=1, d: int=4):
    ...
    def levenshtein_compare(s1: str, s2: str):
        nonlocal distances
        l1 = distances[s1]
        l2 = distances[s2]
        if l1 != l2:
